...
import type { Credentials, LoginResponse, RegisterPayload, RegisterResponse } from '~/types'
...
export const useAuthApi = () => {
  ...
  /**
   * Perform backend registration request
   */
  const register = async (payload: RegisterPayload): Promise<RegisterResponse> => {
    try {
      const cleanBaseURL = baseURL?.replace(/\/$/, '')
      const fullURL = `${cleanBaseURL}/${API_ENDPOINTS.REGISTER}`
      ...
      console.error('❌ Auth API register error:', err.message || err)
      const errorMsg = err?.data?.message || err?.message || 'Unknown error occurred'
      return {
        success: false,
        message: errorMsg,
